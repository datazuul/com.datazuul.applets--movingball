...
/**
 * Diese Klasse enthält alle für die Implementierung eines Applets wichtigen
 * Methoden (init, start, stop), desweiteren die Methoden run (zur
 * Implementierung des Interfaces Run- abble), sowie die Graphikmethoden paint
 * und update. Die Klasse realisiert die Dopppel- pufferung in der Methode
 * update, sie hört auf Mausereignisse (mouseDown) und enthält alle für den
 * Ablauf des Spiels wichtigen Anweisungen
 */
...
public class Main extends Applet implements Runnable {
	...
	boolean isStoped = true; // Zeigt an, ob das Spiel gestopt ist (true) oder
								// läuft (false)
	...
	// Audiodateien
	AudioClip shotnoise; // Speichert die Wav - Datei Gun, die nach Schuss
							// abgespielt wird
	AudioClip hitnoise; // Speichert die Wav - Datei hit, die nach einem Treffer
						// abgespielt wird
	AudioClip outnoise; // Speichert die Wav - Datei error, die nach einem
						// Treffer abgespielt wird
	...
	public void init() {
		...
		// Laden der Bilder und Audiodateien und einmaliges Abspielen, um
		// längere Ladezeiten während des Spiels zu vermeiden
		hitnoise = getAudioClip(getCodeBase(), "shootball/gun.au");
		hitnoise.play();
		hitnoise.stop();
		shotnoise = getAudioClip(getCodeBase(), "shootball/miss.au");
		shotnoise.play();
		shotnoise.stop();
		outnoise = getAudioClip(getCodeBase(), "shootball/error.au");
		outnoise.play();
		outnoise.stop();
	...
	public void paint(Graphics g) {
		...
		// Wenn alle Leben verbraucht sind
			...
			g.drawString("You scored " + player.getScore() + " Points!", 90,
					140);
			...
			if (player.getScore() < 300)
				...
				g.drawString("You are the Champion!", 100, 190);

			g.drawString("Doubleclick on the Applet, to play again!", 20, 220);

			isStoped = true; // Zurücksetzen der isStoped Variablen, um wieder
